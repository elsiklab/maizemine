...
return declare( ActionBarDialog, {
    ...
    _dialogContent: function () {
        ...
        // Render 'treat as regex' checkbox
        var iterateDiv = dom.create("div", {
            className: "checkboxdiv"
        }, textOptionsDiv );
        content.regex_iterate = new dCheckBox({
                                        label: "Iterate regular expression",
                                        id: "iterate_regex"
                                    }).placeAt( iterateDiv );
        dom.create( "label", { "for": "iterate_regex", innerHTML: "Iterate regular expression" }, iterateDiv );
    ...
    _getSearchParams: function() {
        ...
        return {
            ...
            iterate: content.regex_iterate.checked,
            ...
            array.forEach( sequences, function( r ) {
                if( params.translate ) {
                    for( var frameOffset = 0; frameOffset < 3; frameOffset++ ) {
                        this._searchSequence( query, r[0], expr, r[1], featCallback, params.iterate, true, frameOffset );
                    }
                } else {
                    this._searchSequence( query, r[0], expr, r[1], featCallback, params.iterate );
                }
            }, this );
        },

        _searchSequence: function( query, sequence, expr, strand, featCallback, iterate, translated, frameOffset ) {
            if( translated )
